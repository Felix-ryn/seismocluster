...
export default function Movement() {
  ...
  const clusterCounts = useMemo(() => {
    ...
    data.forEach((d) => {
      const key = d.is_anomaly
        ? "Anomali"
        : `Cluster ${d.cluster_label ?? d.cluster_id ?? "?"}`;
  ...
  return (
    <div>
      ...
      {!loading && !error && data.length === 0 && (
        <p className="empty-state">Belum ada data clustering. Jalankan ETL dan Training terlebih dahulu.</p>
      )}

      {data.length > 0 && (
        <>
          {/* Chart 1: Scatter + Zona Kedalaman */}
          <div className="panel">
            ...
            <div style={{ height: 360 }}>
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 4, right: 24, left: 0, bottom: 4 }}>
                  ...
                  <XAxis
                    dataKey="magnitude" name="Magnitudo" type="number" domain={["auto", "auto"]}
                    tick={{ fontSize: 9, fill: "#94A3B8" }}
                    label={{ value: "Magnitudo (SR)", position: "insideBottomRight", offset: -10, fontSize: 9, fill: "#94A3B8" }}
                  />
                  <YAxis
                    dataKey="depth" name="Kedalaman" type="number" domain={["auto", "auto"]}
                    tick={{ fontSize: 9, fill: "#94A3B8" }}
                  ...
                  <Tooltip
                    ...
                    formatter={(val, name) => [name === "Magnitudo" ? `${val} SR` : `${val} km`, name]}
                  />
                  <ReferenceLine y={70} stroke="rgba(251,191,36,0.6)" strokeDasharray="4 2"
                    label={{ value: "Dangkal / Menengah (70 km)", position: "insideTopRight", fontSize: 9, fill: "rgba(251,191,36,0.9)" }} />
                  <ReferenceLine y={300} stroke="rgba(249,115,22,0.6)" strokeDasharray="4 2"
                    label={{ value: "Menengah / Dalam (300 km)", position: "insideTopRight", fontSize: 9, fill: "rgba(249,115,22,0.9)" }} />
                  {Object.entries(grouped).map(([key, points]) => (
                    <Scatter key={key} name={getName(key)} data={points} fill={getColor(key)} fillOpacity={0.72} />
                  ))}
                </ScatterChart>
              </ResponsiveContainer>
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
            ...
            {/* Chart 2: Jumlah per Cluster */}
            <div className="panel">
              <p className="panel-title">Jumlah Gempa per Cluster</p>
              <div style={{ height: 220 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={clusterCounts} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
                    <XAxis dataKey="name" tick={{ fontSize: 9, fill: "#94A3B8" }} />
                    <YAxis tick={{ fontSize: 9, fill: "#94A3B8" }} />
                    <Tooltip
                      contentStyle={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.10)", borderRadius: "8px", fontSize: "12px", color: "#1e293b" }}
                      formatter={(val) => [`${val} gempa`, "Jumlah"]}
                    />
                    <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                      {clusterCounts.map((entry) => {
                        const key = entry.name === "Anomali" ? "anomaly" : entry.name.replace("Cluster ", "");
                        return <Cell key={entry.name} fill={getColor(key)} fillOpacity={0.85} />;
                      })}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
            ...
            {/* Chart 3: Frekuensi per Bulan */}
            <div className="panel">
              <p className="panel-title">Frekuensi Gempa per Bulan</p>
              <div style={{ height: 220 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
                    ...
                    <Tooltip
                      contentStyle={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.10)", borderRadius: "8px", fontSize: "12px", color: "#1e293b" }}
                      formatter={(val) => [`${val} gempa`, "Frekuensi"]}
                    />
                    <Bar dataKey="count" fill="#7C3AED" fillOpacity={0.8} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
